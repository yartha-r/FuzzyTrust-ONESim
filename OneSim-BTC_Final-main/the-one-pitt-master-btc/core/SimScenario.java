...
public class SimScenario implements Serializable {
    ...
    /**
     * Intialize DT, IT, and Sus between verificator and every host
     */
    public void initQLearn() {
    ...
    public void readFuzzyFile(String filename) {
    ...
    protected void createHosts() {
        ...
        for (int i = 1; i <= nrofGroups; i++) {
            ...
            if (s.contains(COOPFACTOR)) {
                coopF = s.getDouble(COOPFACTOR);
            } else {
                coopF = 0.0;
            }

            if (s.contains(NEGFACTOR)) {
                negF = s.getDouble(NEGFACTOR);
            } else {
                negF = 0.0;
            }
            ...
            for (int j = 0; j < nrofHosts; j++) {
                ...
                host.setInitBalance((float) balance);
                host.setCoopFactor(coopF);
                host.setNegativeFactor(negF);
                hosts.add(host);

                //Koko was here
                /**
                 * If host includes in group Misbe then add to misbehaviour Node
                 * lists
                 */
    ...
    //Koko was here
    /**
     * Returns the list of Misbehaves nodes for this scenario.
     *
     * @return the list of Misbehaves nodes for this scenario.
     */
