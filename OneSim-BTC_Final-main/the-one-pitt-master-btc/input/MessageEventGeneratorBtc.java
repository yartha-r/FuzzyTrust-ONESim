 ...
 * be configured.
 */
public class MessageEventGeneratorBtc implements EventQueue {
	
        public static final String MESSAGE_PRICE = "price";
	 ...
	 */
	public MessageEventGeneratorBtc(Settings s){
		this.sizeRange = s.getCsvInts(MESSAGE_SIZE_S);
		this.priceRange = s.getCsvDoubles(MESSAGE_PRICE);
		...
		if (s.contains(TO_HOST_RANGE_S)) {
			this.toHostRange = s.getInt(TO_HOST_RANGE_S);
		}
